...
interface SearchResult { chainId: number; agentId: string; name: string; image: string; trustScore: number; }
...
export default function Navbar() {
  ...
  const statusDot =
    socketState === 'open' ? 'bg-success shadow-[0_0_8px_var(--color-success)]' :
    socketState === 'connecting' || socketState === 'reconnecting' ? 'bg-accent shadow-[0_0_8px_var(--color-accent)]' :
    'bg-muted';
  const statusLabel =
    socketState === 'open' ? 'Live' :
    ...
    <nav className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border text-sm px-6 h-16 w-full flex items-center justify-between shadow-2xl">
      <div className="flex items-center gap-6">
        ...
        <Link href="/" className="flex items-center gap-2 text-primary font-bold text-lg">
          ...
          <span className="text-white">ERC<span className="text-primary">-8004</span></span>
        </Link>
        ...
        <div className="flex items-center gap-4 border-l border-border pl-6 ml-2">
          ...
          <Link href="/penalties?tab=indexer" className="flex items-center gap-2 text-muted hover:text-white transition-colors">
            <Activity size={15} /> System
          </Link>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div ref={searchRef} className="relative">
          ...
          {showSearch && (results.length > 0 || searching) && (
            <div className="absolute top-full mt-2 w-[300px] right-0 bg-card border border-border rounded-xl shadow-2xl overflow-hidden py-2" style={{ zIndex: 100 }}>
              {searching && <div className="px-4 py-3 text-muted text-sm border-b border-border">Searching…</div>}
              {results.map(r => (
                <button key={r.agentId} className="w-full text-left flex items-center gap-3 px-4 py-2 hover:bg-white/5 transition-colors"
                  onClick={() => { router.push(`/agents/${r.chainId}/${r.agentId}`); setShowSearch(false); setQuery(''); }}>
                  <img src={resolveIPFS(r.image)} alt={r.name} className="w-8 h-8 rounded-full border border-border" onError={e => { (e.target as HTMLImageElement).src = 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"%3E%3Crect width="100" height="100" fill="%231E293B"/%3E%3C/svg%3E'; }} />
                  <div className="flex flex-col flex-1 overflow-hidden">
                    <span className="text-white text-sm font-medium truncate">{r.name}</span>
                    <span className="text-primary text-xs font-bold leading-none mt-1">Trust Score: {r.trustScore.toFixed(1)}</span>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-card border border-border" title={`Realtime stream: ${statusLabel}`}>
